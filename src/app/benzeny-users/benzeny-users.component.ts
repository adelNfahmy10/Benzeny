...
@Component({
  selector: 'app-benzeny-users',
  standalone: true,
  imports: [BtnSaveComponent, NgxCustomModalComponent, IconModule, ReactiveFormsModule, NgClass, DataTableModule, FormsModule, BtnAddComponent],
  templateUrl: './benzeny-users.component.html',
  styleUrl: './benzeny-users.component.css'
})
export class BenzenyUsersComponent {
    ...
    getAllUsers():void{
        this._UserService.GetBenzenyUsers().subscribe({
            next:(res)=>{
                ...
                this.totalUsers.set(res.data.count)
    ...
    getAllRoles():void{
        this._RolesService.GetAllRoles().subscribe({
            next:(res)=>{
                const excludedRoles = ['Admin', 'CompanyOwner', 'BranchManager', 'Benzeny', 'Driver', 'Finance'];
                const roles = res.data.filter((role: any) => !excludedRoles.includes(role.name));
    ...
    chagneUserRole():void{
        this.userForm.value.roleIds = this.selectedRoles();
        let data = {
            userId: this.userId,
            roles: this.userForm.value.roleIds
        }
        this._UserService.changeUserRole(data).subscribe(res => {
            const toast = Swal.mixin({
                toast: true,
                position: 'top-end',
                showConfirmButton: false,
            ...
            toast.fire({
                icon: 'success',
                title: 'Change User Role Is successfully',
                padding: '10px 20px',
            });
            this.userForm.reset()
            this.getAllUsers();
